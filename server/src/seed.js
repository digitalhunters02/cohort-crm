...
async function main() {
  ...
  const { count: existingInquiries } = await get(`SELECT COUNT(*) AS count FROM inquiries`);
  ...
  for (const i of inquiriesData) {
    inquiryIds.push((await run(`
      ...
      VALUES (@student_name, @grade_applying_for, @parent_name, @parent_email, @parent_phone, @source, @status, @owner_user_id, @created_at, @notes) RETURNING id
    `, i)).rows[0].id);
  }
  const [
  ...
  for (const a of applicantsData) {
    applicantIds.push((await run(`
      ...
      VALUES (@inquiry_id, @student_name, @grade_applying_for, @parent_name, @parent_email, @parent_phone, @stage, @owner_user_id, @application_date, @decision_date, @notes) RETURNING id
    `, a)).rows[0].id);
  }
  const [
  ...
  for (const t of toursData) {
    tourIds.push((await run(`
      INSERT INTO tours_events (type, title, date, capacity, attendees_count, location, notes)
      VALUES (@type, @title, @date, @capacity, @attendees_count, @location, @notes) RETURNING id
    `, t)).rows[0].id);
  }
  ...
  for (const f of familiesData) {
    familyIds.push((await run(`
      INSERT INTO families (primary_guardian_name, secondary_guardian_name, email, phone, address, notes)
      VALUES (@primary_guardian_name, @secondary_guardian_name, @email, @phone, @address, @notes) RETURNING id
    `, f)).rows[0].id);
  }
